...
impl ElvesGroup {
    ...
    pub fn get_repeated_item(&self) -> String {
        ...
        let aux = existing_chars
            .iter()
            .find(|(_character, ocurrences)| **ocurrences == self.rucksacks.len())
            .unwrap();
...
pub fn get_input() -> Vec<ElvesGroup> {
    let input = vec![
        ...
        "≥vVGtv÷McjLVGHfHDrPPWZppBpJpfZZZ",
...
pub fn get_answer() {
    ...
    println!(
        "Total sum of item types for elves group: {:?}",
        elves_groups
            .iter()
            .map(|group| group.get_badge_priority())
            .collect::<Vec<i32>>()
            .iter()
            .sum::<i32>()
    );
